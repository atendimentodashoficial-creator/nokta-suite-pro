...
function SortableProfissionalItem({ 
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id, disabled: !isActive });
  ...
  return (
    ...
    >
      ...
      <Switch
        checked={isActive}
        onCheckedChange={(checked) => onToggle(profissionalId, checked, vinculoId)}
        disabled={isPending}
      />
    </div>
  );
...
export default function VinculosProcedimentos() {
  ...
  const handleDragEnd = (procedimentoId: string, profissionaisAtivos: typeof procedimentosComProfissionais[0]['profissionais']) => 
    (event: DragEndEvent) => {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {totalVinculos} vínculo(s) ativo(s) • Ative os profissionais que podem realizar cada procedimento
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="text-center py-8 text-muted-foreground">Carregando...</div>
          ) : procedimentosComProfissionais.length === 0 ? (
                ...
                return (
                  <div key={item.procedimento.id} className="rounded-lg border p-4">
                    ...
                    <DndContext
                      ...
                      onDragEnd={handleDragEnd(item.procedimento.id, item.profissionais)}
                    >
                      <SortableContext
                        items={activeProfissionais.map((p) => p.profissionalId)}
